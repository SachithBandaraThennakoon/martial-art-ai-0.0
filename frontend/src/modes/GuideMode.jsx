...
function PhaseNavigator({ phases, selected, onSelect, playingPhase = null }) {
  return <nav className="guide-progress" aria-label="Technique phases">{phases.map((phase, index) => { const isSelected = selected === index; const isPlaying = playingPhase === index; return <button aria-current={isSelected ? "step" : undefined} className={`${isSelected ? "is-active" : ""} ${isPlaying ? "is-playing" : ""}`.trim()} key={`${phase.step_number}-${index}`} onClick={() => onSelect(index)} type="button"><span>{String(index + 1).padStart(2, "0")}</span><strong>{phase.step_name}</strong>{isPlaying ? <small>Now playing</small> : isSelected ? <small>Selected lesson</small> : null}</button>; })}</nav>;
}
...
export default function GuideMode({ isAdminStudio = false, selectedTechniqueName = "" }) {
  ...
  const [playbackPhase, setPlaybackPhase] = useState(0);
  ...
  const requiredMeasurements = measurements.filter((measurement) => measurement.mastery_required === true);
  ...
  const playFullMovement = () => {
    ...
    window.requestAnimationFrame(() => document.querySelector(".guide-skeleton")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  };
  const activePlaybackPhase = Math.min(playbackPhase, phases.length - 1);
  return <section className="guide-mode" ref={guideRef}>
    <header className="guide-mode__hero guide-mode__hero--progressive"><div><p className="eyebrow">{isAdminStudio ? "Admin preview · Technique guide" : "Technique guide"}</p><h1>{name}</h1><p>{overview.summary || state.data.description || "Learn the movement from the complete action through its key phases."}</p>{purpose ? <p className="guide-mode__purpose"><strong>Purpose</strong>{purpose}</p> : null}<button className="btn btn--light" onClick={playFullMovement} type="button">Play full movement</button></div><div className="guide-mode__meta"><span>{difficulty}</span><span>{phases.length} phases</span><span>{learningTime ? `~${learningTime} min guide` : "Progressive guide"}</span></div></header>
    <section className="guide-mode__understand"><div className="guide-mode__section-heading"><div><p className="eyebrow">Understand</p><h2>See the whole movement first</h2></div><span>{isPlaying ? `Playing step ${activePlaybackPhase + 1} of ${phases.length}` : `Lesson ${selectedPhase + 1} of ${phases.length}`}</span></div><PhaseNavigator phases={phases} playingPhase={isPlaying ? activePlaybackPhase : null} selected={selectedPhase} onSelect={setSelectedPhase} /><div className="guide-mode__main"><GuideSkeletonViewer animation={content.animation || {}} autoplayRequest={autoplayRequest} onPhaseChange={setSelectedPhase} onPlaybackPhaseChange={setPlaybackPhase} onPlaybackStateChange={setIsPlaying} selectedPhaseIndex={selectedPhase} steps={steps} /><aside className="guide-mode__objectives"><p className="eyebrow">Movement objectives</p><ol>{objectives.map((objective) => <li key={objective}>{objective}</li>)}</ol><div className="guide-mode__safety"><strong>Practice safely</strong>{list(overview.safety).map((item) => <p key={item}>{item}</p>)}</div></aside></div>{isPlaying ? <div aria-live="polite" className="guide-mode__playback-context"><div><span>Now playing</span><strong>Step {activePlaybackPhase + 1}: {phases[activePlaybackPhase].step_name}</strong></div><div><span>Lesson below</span><strong>Step {selectedPhase + 1}: {phase.step_name}</strong></div>{activePlaybackPhase !== selectedPhase ? <button onClick={() => setSelectedPhase(activePlaybackPhase)} type="button">Pause and open this lesson</button> : <small>The lesson stays still while the movement plays.</small>}</div> : null}</section>
    <section className="guide-mode__phase-lesson"><div className="guide-mode__section-heading"><div><p className="eyebrow">Learn one phase at a time</p><h2>Step {Math.min(selectedPhase + 1, phases.length)} of {phases.length}: {phase.step_name}</h2></div><div className="guide-mode__phase-actions"><button disabled={selectedPhase === 0} onClick={() => setSelectedPhase((value) => Math.max(0, value - 1))} type="button">Previous phase</button><button disabled={selectedPhase >= phases.length - 1} onClick={() => setSelectedPhase((value) => Math.min(phases.length - 1, value + 1))} type="button">Next phase</button></div></div><div className="guide-mode__lesson-grid"><article><span className="eyebrow">Goal</span><p>{phaseGoal}</p>{phase.remember ? <small><b>Remember:</b> {phase.remember}</small> : null}{phase.why_it_matters ? <small><b>Why it matters:</b> {phase.why_it_matters}</small> : null}</article><article><span className="eyebrow">Focus on</span>{cues.length ? <ul>{cues.map((cue) => <li key={cue}>{cue}</li>)}</ul> : <p>Follow the reference phase and keep the movement controlled.</p>}</article>{requiredMeasurements.length ? <article><span className="eyebrow">Ready to move on</span><ul>{requiredMeasurements.map((target) => <li key={target.body_part}>{target.label}: {target.min}°–{target.max}°</li>)}</ul></article> : null}{list(phase.body_cues).length ? <article><span className="eyebrow">Body cues</span><ul>{phase.body_cues.map((cue) => <li key={cue}>{cue}</li>)}</ul></article> : null}{list(phase.what_to_feel).length ? <article><span className="eyebrow">What to feel</span><ul>{phase.what_to_feel.map((cue) => <li key={cue}>{cue}</li>)}</ul></article> : null}{list(phase.avoid).length ? <article><span className="eyebrow">Avoid</span><ul>{phase.avoid.map((cue) => <li key={cue}>{cue}</li>)}</ul></article> : null}</div>{phase.transition_condition ? <p className="guide-mode__transition"><b>Transition when:</b> {phase.transition_condition}</p> : null}</section>
    ...
    <section className="guide-mode__technical"><div className="guide-mode__section-heading"><div><p className="eyebrow">Progressive disclosure</p><h2>Technical details</h2></div><div className="guide-mode__view-switcher" role="tablist">{["beginner", "technical", "research"].map((level) => <button aria-selected={detailLevel === level} className={detailLevel === level ? "is-active" : ""} key={level} onClick={() => setDetailLevel(level)} role="tab" type="button">{level[0].toUpperCase() + level.slice(1)}</button>)}</div></div>{detailLevel === "beginner" ? <p className="guide-mode__technical-note">Start with goals, cues, errors, and safety. Measurements are available when you are ready.</p> : <><div className="guide-mode__measurement-grid">{measurements.length ? measurements.map((measurement, index) => <article key={`${measurement.body_part}-${index}`}><span>{measurement.label || measurement.body_part?.replaceAll("_", " ")}</span><strong>{measurement.min != null && measurement.max != null ? `${measurement.min}°–${measurement.max}°` : "Reference available"}</strong><small>{measurement.mastery_required ? "Required for progression" : phase.step_name}</small></article>) : <p>No technical measurements are available for this phase.</p>}</div>{detailItems.length ? <div className="guide-mode__detail-list">{detailItems.map((item) => <article key={item.label}><strong>{item.label}</strong><p>{item.detail}</p></article>)}</div> : null}</>}</section>
